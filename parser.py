...
# Таймауты укорочены (было connect=10/read=15/write=10/pool=10). При живой
# куке сайт отвечает за доли секунды — 8 секунд с запасом хватает. Это не
# чинит саму причину зависаний, но не даёт КАЖДОЙ группе ждать по 15+ секунд,
# если кука уже не работает.
...
def make_client(cookie: str) -> httpx.AsyncClient:
    ...
    # Если вставлена полная Cookie-строка: PHPSESSID=xxx; cookie2=yyy
    if "=" in cookie:
...
async def fetch_schedule(cookie: str, group_id: int, date_str: str | None = None) -> list[dict]:
    ...
    logger.info("========================================")
    logger.info("API REQUEST group=%s date=%s", group_id, date_str)
    ...
    try:
        async with make_client(cookie) as client:
            ...
            logger.info("API RESPONSE group=%s status=%s", group_id, response.status_code)
            ...
            logger.info("group=%s organizations=%s", group_id, len(organizations))
            ...
            for organization in organizations:
                ...
                logger.info(
                    "group=%s organization lessons=%s", group_id, len(organization_lessons)
                )
        ...
        logger.error("group=%s HTTP STATUS ERROR: %s", group_id, e, exc_info=True)
        return []
    except httpx.HTTPError as e:
        logger.error("group=%s HTTP ERROR: %s", group_id, e, exc_info=True)
        return []
    except Exception as e:
        logger.error("group=%s UNEXPECTED ERROR: %s", group_id, e, exc_info=True)
        return []
    ...
    try:
        async with make_client(cookie) as client:
            response = await client.get(
                API_URL,
                params={"act": "schedule", "date": date_str, "groupId": sample_group_id},
            )
            logger.info("COOKIE CHECK status=%s url=%s", response.status_code, response.url)
            logger.info("COOKIE CHECK BODY:\n%s", response.text[:2000])
